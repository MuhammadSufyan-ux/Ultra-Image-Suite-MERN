...
interface SquareCropToolProps {
  onBack: () => void;
}

const SquareCropTool: React.FC<SquareCropToolProps> = ({ onBack }) => {
  ...
  const executeCrop = async () => {
    ...
    setCroppedImage(canvas.toDataURL('image/jpeg', 0.98));
  ...
  return (
    <div className="animate-in fade-in duration-500 max-w-6xl mx-auto pb-20 px-4">
      ...
      <div className="text-center mb-8">
        <h1 className="text-2xl font-black text-slate-800 uppercase tracking-tight">Square Crop Image (1:1)</h1>
        ...
        {!selectedImage ? (
          <div className="flex-1 flex items-center justify-center p-20">
            <div onClick={() => fileInputRef.current?.click()} className="w-full max-w-2xl border-2 border-dashed border-[#c5cae9] rounded-[8px] p-24 text-center hover:bg-slate-50 cursor-pointer transition-all group bg-white flex flex-col items-center justify-center">
              <i className="fas fa-square text-6xl text-indigo-100 mb-8 group-hover:scale-110 transition-transform"></i>
              <h3 className="text-[14px] font-black text-slate-700 uppercase tracking-widest mb-2">Select Image to Square Crop</h3>
              <button className="px-12 py-3.5 bg-[#00796b] text-white rounded-[4px] font-black text-[11px] uppercase tracking-widest shadow-xl">Choose Image</button>
          ...
          <div className="flex flex-col lg:flex-row flex-1">
            <div className="flex-1 bg-slate-100 p-6 md:p-10 flex flex-col items-center justify-center relative overflow-hidden">
              <div className="text-[10px] font-black uppercase text-slate-400 mb-4 tracking-widest">Adjust 1:1 Square Frame</div>
              ...
              <div className="relative shadow-2xl bg-white border-4 border-white select-none overflow-hidden max-w-full">
                <div className="relative" style={{ transform: `scale(${zoom})`, transition: 'transform 0.2s' }}>
                  ...
                  <div 
                    ...
                    className="absolute cursor-move border-2 border-white flex items-center justify-center z-20"
                    style={{ 
                  ...
                  >
                     <img src={selectedImage} className="max-w-none block absolute pointer-events-none" 
                      ...
                      <div className="absolute inset-0 border border-white/20 pointer-events-none grid grid-cols-3 grid-rows-3">
                         {[1,2,3,4,5,6,7,8,9].map(i => <div key={i} className="border border-white/10"></div>)}
                      </div>
                  </div>
                  ...
                  <div 
                    ...
                    className="absolute w-5 h-5 bg-white border-2 border-[#3f51b5] rounded-sm z-30 cursor-nwse-resize shadow-md"
            ...
            <div className="w-full lg:w-80 p-8 bg-slate-50 border-l py7-border-default flex flex-col items-center">
               {croppedImage ? (
                 <div className="w-full space-y-6 animate-in slide-in-from-right-4">
                    <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest border-b pb-2">Result</p>
                    <div className="bg-white p-2 border py7-border-default rounded-[4px] shadow-xl">
                       <img src={croppedImage} className="w-full h-auto rounded-sm" />
                    </div>
                    <button onClick={() => { const a = document.createElement('a'); a.href = croppedImage; a.download = `py7-square-${Date.now()}.jpg`; a.click(); }} className="w-full py-4 bg-[#00796b] text-white rounded-[4px] font-black text-[12px] uppercase shadow-xl hover:bg-[#004d40]">Download Image</button>
                 </div>
               ) : (
                 <div className="flex-1 flex flex-col items-center justify-center text-slate-300 opacity-50">
                    <i className="fas fa-crop text-4xl mb-4"></i>
                    <p className="text-[9px] font-black uppercase tracking-widest text-center">Preview will appear here after crop</p>
                 </div>
               )}
